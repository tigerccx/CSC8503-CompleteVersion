...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath) {
	...
	//NOTE: This thingy starts from 0 and expands to x_pos and z_pos
	...
	NavigationNodeOpenList  openList(CmpGridNodePtr);
	NavigationNodeCloseList  closedList;
...
bool NavigationGrid::NodeInOpenList(GridNode* n, NavigationNodeOpenList& list) const {
	auto i = std::find_if(list.begin(), list.end(), [&](GridNode* a)->bool { return a == n; });
	return i == list.end() ? false : true;
}

bool NCL::CSC8503::NavigationGrid::NodeInCloseList(GridNode* n, NavigationNodeCloseList& list) const
{
	auto i = std::find(list.begin(), list.end(), n);
	return i == list.end() ? false : true;
}

GridNode*  NavigationGrid::RemoveBestNode(NavigationNodeOpenList& list) const {
	auto bestI = list.begin();
	GridNode* bestNode = *bestI;
	list.erase(bestI);
...
bool NCL::CSC8503::CmpGridNodePtr(const GridNode* a, const GridNode* b)
{
	return (a->f < b->f);
}
