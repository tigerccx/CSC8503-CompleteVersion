...
namespace NCL {
	...
	namespace CSC8503 {
		enum QuadTreeChildNode {
			QuadTreeChildNode_NX_Z,
			QuadTreeChildNode_X_Z,
			QuadTreeChildNode_NX_NZ,
			QuadTreeChildNode_X_NZ
		};
		...
		protected:
			...
			void Insert(T& object, const Vector3& objectPos, const Vector3& objectSize, int depthLeft, int maxSize) {
				if (!CollisionDetection::AABBTest(objectPos, Vector3(position.x, 0, position.y), objectSize, Vector3(size.x, 1000.0f, size.y))) { //To insert into this node?
					return;
				}
			...
			void Split() {
				...
				children[0] = QuadTreeNode<T>(position + Vector2(-halfSize.x, halfSize.y), halfSize); //-x, z
				children[1] = QuadTreeNode<T>(position + Vector2(halfSize.x, halfSize.y), halfSize); //x, z
				children[2] = QuadTreeNode<T>(position + Vector2(-halfSize.x, -halfSize.y), halfSize); //-x, -z
				children[3] = QuadTreeNode<T>(position + Vector2(halfSize.x, -halfSize.y), halfSize); //x,-z
			}

			void DebugDraw() {
				Debug::DrawAABB(Vector3(position.x, 0, position.y), Vector3(size.x, 1000.0f, size.y), Vector4(0, 0, 1, 1), 10.0f);
				if (children) {
					for (int i = 0; i < 4; ++i) {
			...
			void OperateOnContents(QuadTreeFunc& funcUpdate) {
				if (children) {
					for (int i = 0; i < 4; ++i) {
		...
		protected:
			...
			Vector2 position;
			Vector2 size; // Note!! This is half size. Not size.
...
namespace NCL {
	...
	namespace CSC8503 {
		...
		public:
			...
			void OperateOnContents(typename QuadTreeNode<T>::QuadTreeFunc  funcUpdate) {
				root.OperateOnContents(funcUpdate);
			}
