...
void PhysicsSystem::SetLinearDamping(const float damping) {
	linearDamping = damping;
}

float PhysicsSystem::GetLinearDamping() const {
	return linearDamping;
}

void PhysicsSystem::SetAngularDamping(const float damping) {
	angularDamping = damping;
}

float PhysicsSystem::GetAngularDamping() const {
...
void NCL::CSC8503::PhysicsSystem::RemoveAllCollisionsInCurrentWorld()
{
	GameObjectIterator beg, end;
	gameWorld->GetObjectIterators(beg, end);
	std::set<GameObject*> gos;
	for (auto itr = beg; itr != end; ++itr) {
		gos.insert(*itr);
	}

	for (auto itr = allCollisions.begin(); itr != allCollisions.end();) {
		if (gos.find(itr->a) != gos.end() || gos.find(itr->b) != gos.end()) {
			itr = allCollisions.erase(itr);
		}
		else
		{
			++itr;
...
void PhysicsSystem::BasicCollisionDetection() {
	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (Layer::CheckBetweenLayersDetection((*i)->GetLayerName(), (*j)->GetLayerName())) { //Check if between layer detection is enabled
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float cRestitution = physA->GetElasticity() * physB->GetElasticity(); //0.66 * ;
...
void NCL::CSC8503::PhysicsSystem::PenaltyResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const
{
	...
	physA->AddForceAtPosition(force, p.localA + transformA.GetPosition());
	physB->AddForceAtPosition(-force, p.localB + transformB.GetPosition());
}
...
void PhysicsSystem::BroadPhase() {
	...
	OcTree<GameObject*>* treeDynamic = gameWorld->GetHierarchy()->treeDynamic;
	OcTree<GameObject*>* treeStatic = gameWorld->GetHierarchy()->treeStatic;

	treeDynamic->OperateOnContents(
		[&](std::list<OcTreeEntry<GameObject*>>& data) {
			CollisionDetection::CollisionInfo info;
			for (auto i = data.begin(); i != data.end(); ++i) {
				GameObject* goI = i->object;
				if (goI->GetPhysicsObject() == nullptr)
					continue;

				for (auto j = std::next(i); j != data.end(); ++j) {
					//is this pair of items already in the collision set -
					//if the same pair is in another quadtree node together etc
					GameObject* goJ = j->object;
					if (goJ->GetPhysicsObject() == nullptr)
						continue;
					if (Layer::CheckBetweenLayersDetection(goI->GetLayerName(), goJ->GetLayerName())) {
						info.a = min(goI, goJ);
						info.b = max(goI, goJ);
				...
				goI->GetBroadphaseAABB(halfSize);
				vector<GameObject*> gos;
				CollisionDetection::AABBTreeIntersection(i->object->GetTransform().GetPosition(), halfSize, *treeStatic, gos);
				for (auto j = gos.begin(); j != gos.end(); ++j) {
					if ((*j)->GetPhysicsObject() == nullptr)
						continue;
					if (Layer::CheckBetweenLayersDetection(goI->GetLayerName(), (*j)->GetLayerName())) {
						info.a = goI;
						info.b = *j;
						broadphaseCollisions.insert(info);
					}
...
The broadphase will now only give us likely collisions, so we can now goI through them,
